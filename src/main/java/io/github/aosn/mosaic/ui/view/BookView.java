...
public class BookView extends CustomComponent implements View {
    ...
    public void enter(ViewChangeListener.ViewChangeEvent event) {
        ...
        // Lookup
        var stock = stockService.get(stockId);
        if (stock == null) {
    ...
    private Layout createBookLayout(Stock book) {
        ...
        var visibility = new VisibilityIndicator(book.getVisibility(), i18n);
        contentPane.addComponent(visibility);
        contentPane.setComponentAlignment(visibility, Alignment.MIDDLE_RIGHT);

        var cover = new Image("", new ExternalResource(book.getThumbnailOrPlaceholder()));
        ...
        var isbn = new Label(book.getIsbn());
        isbn.setCaption(i18n.get("book.column.isbn"));
        form.addComponent(isbn);

        var pageCount = new Label(String.valueOf(book.getPageCount()));
        ...
        if (!Strings.isNullOrEmpty(book.getPublishedDate())) {
            var publishedDate = new Label(book.getPublishedDate());
            publishedDate.setCaption(i18n.get("book.column.published.date"));
            form.addComponent(publishedDate);
        }

        var progress = new HorizontalLayout(new ProgressLabel(book, i18n));
        ...
        var reviewContent = book.getLongText();
        var review = new Label("", ContentMode.HTML);
        ...
        if (userService.isLoggedIn()) {
            var backButton = new Button(i18n.get("books.label.title"),
                    e -> getUI().getNavigator().navigateTo(BooksView.VIEW_NAME));

            var editButton = new Button(i18n.get("book.button.edit"), e -> {
            ...
            var buttonArea = new HorizontalLayout(backButton, editButton);
